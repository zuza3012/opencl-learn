...
int main(int argc, char *argv[]){
    ...
    if (opencl_platforms == NULL){
        std::cerr << "Could not allocate memory for OpenCL's platforms" << std::endl;
        return -1;
    }
    ...
    if (cli_err_num != CL_SUCCESS){
        ...
        opencl_platforms = NULL;
        std::cerr << "Error in reading number of computing devices" << std::endl;
        return -1;
    }
    ...
    if (cli_err_num != CL_SUCCESS){
        std::cerr << "Could not read about devices!" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    std::string file_rel_path = "/../hello.cl";

    std::string path = pwd_path+file_rel_path;
    std::cout << path << std::endl;
    
    const char * k = readFile(path.c_str());
    ...
    if (!cl_kernel_program){
        std::cerr << "Could not create kernel program!" << std::endl;
        std::cerr << "Error code: " << cli_err_num << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (cli_err_num){
        std::cerr << "Could not open kernel!" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (cli_err_num != CL_SUCCESS){
        std::cerr << "Could not read output!" << std::endl;
        exit(1);
    }
